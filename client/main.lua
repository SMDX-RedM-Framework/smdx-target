...
RegisterNetEvent('SMDXCore:Client:OnPlayerUnload')
AddEventHandler('SMDXCore:Client:OnPlayerUnload', function()
    isLoggedIn = false
    PlayerData = {}
end)

RegisterNetEvent('SMDXCore:Client:OnJobUpdate')
AddEventHandler('SMDXCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)

RegisterNetEvent('SMDXCore:Client:SetPlayerData')
AddEventHandler('SMDXCore:Client:SetPlayerData', function(val)
    PlayerData = val
end)
...
RegisterNUICallback('selectTarget', function(option, cb)
    ...
    CreateThread(function()
        ...
        elseif data.event ~= nil then
            ...
            elseif data.type == "smdxcommand" then
                TriggerServerEvent('SMDXCore:CallCommand', data.event, data)
            else
                TriggerEvent(data.event, data)
            end
        else
            print("[smdx-target]: ERROR NO EVENT SETUP")
        end
    end)
...
if Config.Debug then
    AddEventHandler('smdx-target:debug', function(data)
